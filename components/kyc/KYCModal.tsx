...
export const KYCModal: React.FC<KYCModalProps> = ({
  ...
  const handleResetAndUpdate = async () => {
    try {
      ...
      setError(
        err.message || "Failed to reset verification. Please try again.",
      );
      setResetting(false);
    }
  };
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <CoolingOffView
              coolingOffUntil={kycData?.cooling_off_until ?? null}
              onClose={onClose}
            />
          ) : view === "rejected" ? (
            <RejectedView
...
const IntroView: React.FC<{ onStart: () => void }> = ({ onStart }) => (
  <div className="p-4 md:p-8 text-center">
    ...
    <p className="text-sm md:text-base text-gray-400 mb-6 md:mb-8 max-w-md mx-auto">
      To ensure a secure trading environment and comply with regulations, we
      need to verify your identity before you can start trading.
    </p>
...
const PendingView: React.FC<{ onClose: () => void }> = ({ onClose }) => (
  <div className="p-8 text-center">
    ...
    <p className="text-gray-400 mb-8 max-w-md mx-auto">
      Your documents are being reviewed. This usually takes a few minutes, but
      can take up to 24 hours in some cases. We'll notify you once complete.
    </p>
...
const ApprovedView: React.FC<{ onClose: () => void }> = ({ onClose }) => (
  <div className="p-8 text-center">
    ...
    <p className="text-gray-400 mb-8 max-w-md mx-auto">
      Your identity has been verified. You can now access all features of the
...
const CoolingOffView: React.FC<{
  coolingOffUntil: string | null;
  onClose: () => void;
}> = ({ coolingOffUntil, onClose }) => {
  ...
  return (
    <div className="p-8 text-center">
      ...
      <p className="text-gray-400 mb-4 max-w-md mx-auto">
        Your identity has been verified. As a security measure, there's a
        24-hour cooling-off period before you can start trading.
      </p>
...
const ResubmissionView: React.FC<{
  ...
  return (
    <div className="p-8 text-center">
      ...
      <h3 className="text-2xl font-bold text-white mb-4">Action Required</h3>

      <p className="text-gray-400 mb-6 max-w-md mx-auto">
        We need you to fix some issues with your verification. Please review the
...
const UpdateConfirmView: React.FC<{
  ...
  <div className="p-8 text-center">
    ...
    {isResetting ? (
      <p className="text-gray-400 mb-8 max-w-md mx-auto">
        Please wait while we reset your verification status. This will only take
        a moment.
      </p>
    ) : (
      <>
        <p className="text-gray-400 mb-6 max-w-md mx-auto">
          Your identity is currently verified. If you need to update your
          documents (e.g., expired ID, address change), your verification will
          be temporarily reset while we review the new documents.
        </p>
