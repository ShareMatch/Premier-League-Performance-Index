...
import { checkEmailVerificationStatus, checkWhatsAppVerificationStatus } from '../../lib/api';
...
const parsePhoneNumber = (phone: string): { countryCode: string; phoneNumber: string; countryIso: string } => {
  ...
  return { countryCode: '+971', phoneNumber: cleaned.substring(1), countryIso: 'AE' };
};
...
const PhoneInputField = ({
  ...
  const selectedCountry = countries.find((c) => c.code === countryIso) || countries.find(c => c.code === 'AE') || countries[0];
  ...
  return (
    <div className="flex flex-col w-full gap-1 sm:gap-1.5">
      ...
      <div
        className={`flex items-center w-full bg-gray-200 rounded-full shadow-inner transition-all relative h-9 sm:h-10 px-3 sm:px-4 ${error ? 'ring-2 ring-red-500' : 'focus-within:ring-2 focus-within:ring-brand-emerald500'
      ...
      {!error && hint && (
        <p className="text-gray-300 text-[10px] sm:text-xs font-sans ml-1">{hint}</p>
      )}
    </div>
  );
};
...
const EditDetailsModal: React.FC<EditDetailsModalProps> = ({
  ...
  // Phone country state
  const [phoneCountry, setPhoneCountry] = useState({ code: 'AE', dialCode: '+971' });
  const [whatsappCountry, setWhatsappCountry] = useState({ code: 'AE', dialCode: '+971' });
    ...
    if (isOpen) {
      ...
      fields.forEach(field => {
        if (field.type === 'tel') {
          ...
          if (field.key === 'phone') {
            setPhoneCountry({ code: parsed.countryIso, dialCode: parsed.countryCode });
          } else if (field.key === 'whatsapp') {
            setWhatsappCountry({ code: parsed.countryIso, dialCode: parsed.countryCode });
          }
        } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const newWhatsAppE164 = formData.whatsapp 
        ? normalizePhoneToE164(formData.whatsapp, whatsappCountry.dialCode, whatsappCountry.code)
        : '';
      ...
      const currentEmailLower = currentEmail?.trim().toLowerCase();
      console.log('Email check:', { newEmail, currentEmailLower, emailField: !!emailField });
      
      // Check if email is being changed (or if we don't know current email, always check)
      const emailChanged = newEmail && (!currentEmailLower || newEmail !== currentEmailLower);
      ...
      console.log('WhatsApp check:', { newWhatsAppE164, currentWhatsApp, whatsappField: !!whatsappField });
      
      // Check if WhatsApp is being changed (or if we don't know current WhatsApp, always check)
      const whatsappChanged = newWhatsAppE164 && (!currentWhatsApp || newWhatsAppE164 !== currentWhatsApp);
      
      if (whatsappField && newWhatsAppE164 && whatsappChanged) {
        try {
          console.log('Checking WhatsApp status for:', newWhatsAppE164);
          const whatsappStatus = await checkWhatsAppVerificationStatus(newWhatsAppE164, currentUserId);
          ...
          if (whatsappStatus.exists) {
            setFieldErrors(prev => ({ ...prev, whatsapp: 'An account with this WhatsApp number already exists' }));
            setSaving(false);
            return;
          }
        } catch (e) {
      ...
      if (formData.phone) {
        finalData.phone = normalizePhoneToE164(formData.phone, phoneCountry.dialCode, phoneCountry.code);
      }
  ...
  const renderField = (field: EditField) => {
    ...
    return (
      <div key={field.key} className="flex flex-col w-full gap-1 sm:gap-1.5">
        ...
        <div className={`flex items-center w-full bg-gray-200 rounded-full shadow-inner h-9 sm:h-10 px-3 sm:px-4 focus-within:ring-2 ${fieldErrors[field.key] ? 'ring-2 ring-red-500 focus-within:ring-red-500' : 'focus-within:ring-brand-emerald500'} ${field.editable === false ? 'opacity-60 cursor-not-allowed' : ''}`}>
          <input
            id={`edit-${field.key}`}
            type={field.type || 'text'}
            value={formData[field.key] || ''}
            onChange={(e) => handleChange(field.key, e.target.value)}
            onBlur={field.type === 'email' ? (e) => handleEmailBlur(e.target.value) : undefined}
            disabled={field.editable === false}
        ...
        {fieldErrors[field.key] && (
          <p className="text-red-400 text-[10px] sm:text-xs font-sans ml-1">{fieldErrors[field.key]}</p>
        )}
        {!fieldErrors[field.key] && field.hint && (
          <p className="text-gray-300 text-[10px] sm:text-xs font-sans ml-1">{field.hint}</p>
        )}
      </div>
    );
  };
