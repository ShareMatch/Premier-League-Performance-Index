...
const OnThisDay: React.FC<OnThisDayProps> = ({
  assetName,
  market,
  className = "",
}) => {
  ...
  useEffect(() => {
    const fetchFact = async () => {
      ...
      try {
        ...
        const generatedFact =
          data?.fact || `On this day, ${assetName} continues to make history.`;
  ...
  return (
    <div
      className={`bg-gradient-to-br from-[#0B1221] to-[#0f192b] border border-gray-800 rounded-[clamp(0.5rem,1.5vw,0.75rem)] p-[clamp(0.75rem,2vw,1.25rem)] relative overflow-hidden group ${className}`}
    >
