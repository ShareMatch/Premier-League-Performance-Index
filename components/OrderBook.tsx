...
const OrderBook: React.FC<OrderBookProps> = ({ teams, onSelectOrder, onViewAsset, onBack, title }) => {
  return (
    <div data-testid="order-book" className="bg-gray-800/50 rounded-[clamp(0.375rem,1vw,0.5rem)] shadow-2xl shadow-gray-950/50 overflow-hidden border border-gray-700 flex flex-col">
      {/* Back Button & Title */}
      {(onBack || title) && (
        <div className="flex items-center gap-[clamp(0.5rem,1.5vw,0.75rem)] p-[clamp(0.5rem,1.5vw,1rem)] border-b border-gray-700 bg-gray-800/80 backdrop-blur-sm">
          {onBack && (
            <button
              onClick={onBack}
              className="p-[clamp(0.25rem,0.75vw,0.5rem)] rounded-[clamp(0.25rem,0.5vw,0.375rem)] bg-gray-700/50 hover:bg-gray-600/50 text-gray-300 hover:text-white transition-all flex-shrink-0"
              aria-label="Go back"
            >
              <ArrowLeft className="w-[clamp(0.875rem,2vw,1.25rem)] h-[clamp(0.875rem,2vw,1.25rem)]" />
            </button>
          )}
          {title && (
            <h2 className="text-[clamp(0.75rem,2vw,1.125rem)] font-bold text-white">
              {title}
            </h2>
          )}
        </div>
