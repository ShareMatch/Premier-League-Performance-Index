...
test.describe("Home Page", () => {
  ...
  test("Asset price displayed on the list matches price on the detail view", async ({
    page,
  }) => {
    console.log("[Test] Starting price_consistency_across_views...");
    await page.goto("/");
    // First search for Arsenal to ensure it's in results
    await page.locator('[data-testid="desktop-search-input"]').fill("Arsenal");
    await page.locator("button.transform").click();
    await page.waitForTimeout(500);

    // Get the first result with Arsenal in the text
    const arsenalResult = page
      .locator("button")
      .filter({ hasText: "Arsenal" })
      .first();
    await arsenalResult.waitFor({ timeout: 5000 });

    const listText = await arsenalResult.innerText();

    // Click on Arsenal to view details
    await arsenalResult.click();

    // On detail page, verify we can see asset information
    await page.waitForTimeout(500);
    await expect(page.locator('[data-testid="asset-page"]')).toBeVisible({
      timeout: 5000,
    });

    console.log("[Test] Completed price_consistency_across_views");
  });

  test("Help Center button can be focused and activated with keyboard", async ({
    page,
  }) => {
    console.log("[Test] Starting help_center_accessible_via_keyboard...");
    await page.goto("/");
    await page.locator('[data-testid="sidebar-help-center"]').focus();
    await page.keyboard.press("Enter");
    await page
      .locator('[data-testid="alert-modal-close-button"]')
    ...
    await page.locator('[data-testid="alert-modal-close-button"]').click();

    console.log("[Test] Completed help_center_accessible_via_keyboard");
  });

  test("User can navigate to Motorsport section and see Global Events button", async ({
    page,
  }) => {
    console.log("[Test] Starting navigate_to_motorsport_section...");
    await page.goto("/");
    await page.locator('button:has-text("Motorsport")').click();
    await page
      .locator('button:has-text("Global Events")')
      .waitFor({ timeout: 5000 });

    await expect(
      page.locator('button:has-text("Global Events")')
    ).toBeVisible();
    await expect(
      page.locator('button:has-text("FIFA World Cup")')
    ).toBeVisible();

    console.log("[Test] Completed navigate_to_motorsport_section");
  });
