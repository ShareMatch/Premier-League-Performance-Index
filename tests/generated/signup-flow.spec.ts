...
test.describe("Signup Flow", () => {
  ...
  test("Signup with SQL injection attempt", async ({
    page,
    supabaseAdapter,
  }) => {
    console.log("[Test] Starting...");
  ...
  test("Signup with cross-site scripting attempt", async ({
    page,
    supabaseAdapter,
  }) => {
    console.log("[Test] Starting...");
  ...
  test("Complete signup with email and whatsapp verification", async ({
    ...
    const phoneCountryButton = signupModal
    ...
    const phoneCountrySearch = signupModal
      .locator('input[placeholder="Search..."]')
      .first();
    ...
    const whatsappVisible = await whatsappVerificationTitle
    ...
    if (whatsappVisible) {
      ...
      if (whatsappOtp) {
        ...
        const verifyButton = page
          .getByRole("button", { name: /verify/i })
          .first();
        const isButtonVisible = await verifyButton
          .isVisible()
          .catch(() => false);
        const isButtonEnabled = await verifyButton
          .isEnabled()
          .catch(() => false);
