...
async function fetchApplicantVerifiedName(
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        "Failed to fetch applicant info:",
        response.status,
        await response.text()
      );
      return null;
    ...
    const firstName =
      fixedInfo.firstName || info.firstNameEn || info.firstName || "";
    const lastName =
      fixedInfo.lastName || info.lastNameEn || info.lastName || "";
...
serve(async (req) => {
  ...
  try {
    ...
    const SUMSUB_BASE_URL =
      Deno.env.get("SUMSUB_BASE_URL") || "https://api.sumsub.com";
    ...
    const algorithm =
      req.headers.get("x-payload-digest-alg") || "HMAC_SHA256_HEX";
    ...
    if (payload.type === "applicantReviewed") {
      ...
      if (reviewResult?.reviewAnswer === "GREEN") {
        ...
        console.log(
          `Rejection type: ${reviewResult?.reviewRejectType}, status: ${kycStatus}`
        );
      }

      console.log(
        `Updating compliance for user ${externalUserId} KYC status to: ${kycStatus}`
      );
      ...
      if (kycStatus === "approved") {
        complianceUpdate.cooling_off_until = new Date(
      ...
      if (verifiedName) {
        ...
        if (fullNameUpdateError) {
          console.error(
            "Failed to update users.full_name:",
            fullNameUpdateError
          );
        } else {
      ...
      if (verifiedName && authUserId) {
        console.log(
          `Updating auth.users display name for ${authUserId} to: ${verifiedName}`
        );
    ...
    if (payload.type === "applicantCreated") {
      ...
      if (externalUserId && applicantId) {
        ...
        if (complianceError || !compliance) {
          ...
          if (createError) {
            console.error(
              "Failed to init compliance row (applicantCreated):",
              createError
            );
          }
        }
